...
function AjaxCrossDomainRequest(url, method, formname, callback,errorcallback) {
    method = js_strtoupper(method);
	window.AjaxCrossDomainResponse=function() {
		AjaxCrossDomainResponse = FlashHelper.getFlash().GetVariable("retText");
		if (AjaxCrossDomainResponse){
			callback&&callback(AjaxCrossDomainResponse);
		}else{
			alert("Nothing");
	...
	if (method == "GET"){
        ...
        fs.XmlHttp(urlget, "window.AjaxCrossDomainResponse", method, "", contentType);
	} else {
		//POST请求方式
        fs.XmlHttp(url,"window.AjaxCrossDomainResponse", method, body, contentType);
	}
}
