...
define("NONE_VERIFY_CODE","Gentle");
...
//die("<script> window.parent.postMessage('123','*');</script>");
}
...
//die(md5(xsencode($var))." ".$md5);
	if(NONE_VERIFY_CODE&&NONE_VERIFY_CODE==$_REQUEST['nvc']&&$md5==md5(NONE_VERIFY_CODE)){return true;}
    $i=substr(md5(xsencode($var)),8,16)==$md5?true:false;
    if(!$i){
        die(header("HTTP/1.1 403 Forbidden "));
    }
}
//die(var_dump(verify($contents,$_REQUEST["verify"])));
function ajax($url, $data = null, $cookie = null, $referer = null, $proxy = null, $agent = null, $head = null, $timeout=120) {
    $ch = curl_init($url);
    if ($agent) {
    ...
    if($encode){
		...
		if(!$encode[1]){
			 if (function_exists("mb_detect_encoding")) {
				$encode[1]=mb_detect_encoding($str, array('UTF-8', 'ASCII', 'GBK', 'GB2312', 'BIG5', 'JIS', 'eucjp-win', 'sjis-win', 'EUC-JP'));
			}else{
				$encode[1]="UTF-8";
			}
		}
		if(strtolower($encode[1])!=strtolower($encode[0])){
			if (function_exists("mb_convert_encoding")) {
				$str = mb_convert_encoding($str, $encode[0], $encode[1]);
			} else {
				$str=iconv($encode[1],$encode[0]."//IGNORE",$str);
			}
		}
    }
    return $str;
...
function postback($data){
	return "<script>window.parent.postMessage('var code=\"".base64_encode($data)."\";var s=".base64decode($_POST["callback"]).";s(Base64.decode(code));','*');</script>";
}
function preajax($ajax_get_url,$ispost=0){
    verify($ajax_get_url,$_REQUEST["verify"]);
    global $ajax_get_el,$method,$encode,$match,$adv,$type,$cache;
    ...
    if($adv){
        ...
        $dlx->downFile($ajax_get_url, '',1,$flag=array(
            ...
            'data'=>$data?$data:""
        ),$type, $type?false:true, 0 ,1);//$url, $data = null, $cookie = null, $referer = null, $proxy = null, $agent = null, $head = null
        die();
    }
	//die(var_dump(array($url, $data, $ax0[2], $ax0[3], $ax0[4], $ax0[5], $ax0[6])));
    $r = ajax($url, $data, $ax0[2], $ax0[3], $ax0[4], $ax0[5], $ax0[6]);
	//$r=file_get_contents("http://www.baidu.com/");
	//die($r);
    ...
    if($op){
        switch ($op)
        {
            ...
            case "checkfile":
                ...
                break;
            case "postcallback":
                die(postback(preajax(base64decode($_POST["url"]),1)));
                break;
            case "clearall":
...
function xsencode($text,$split=null) {
    $retval = ''; for($i = 0; $i < strlen($text); ++$i) { $s=dechex(ord($text[$i]));$retval .= $split.strlen($s)==1?"0".$s:$s; } return strtoupper($retval);
}
