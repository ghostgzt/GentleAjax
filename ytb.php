<?php
define("MFILE","http://".$_SERVER['HTTP_HOST']."/youtube/watch.mp4");
//define("MFILE","http://ox.ap01.aws.af.cm/ddytb.php");
if($_GET['ytbdownproxy']){
        $p=base64_decode(str_replace(" ","+",strrev($_GET['ytbdownproxy'])));
//die($p);
	ytbdownproxy($p);
	exit;
}
function curl($url,$ref,$cookie,$agent,$proxy,$header,$data){
	$ch = curl_init($url);
...
if($_GET['list']){
	...
	$str=curl($url,$ref,$cookie,$agent,$proxy,$header,$data);
	$s=json_decode($str,1);
	...
	for($i=0;$i<count($s['urls']);$i++){
		echo '<tr><td><img style="max-height:200px;max-width:200px;"src="'.str_replace("http://i.ytimg.com/",$rootUrl."iytimg/",$s['info']['thumbnail']).'"/></td><td>'.$s['urls'][$i]['name'].'</td><td>'.fsize($s['urls'][$i]['size']).'</td><td><a href="'.MFILE.'?ytbdownproxy='.strrev(base64_encode($s['urls'][$i]['url'])).'">播放</a>&nbsp;<a href="'.MFILE.'?download=1&ytbdownproxy='.strrev(base64_encode($s['urls'][$i]['url'])).'">下载</a></td></tr>';
	...
	$imin=0;
	for($i=0;$i<count($s['urls']);$i++){
		if($s['urls'][$i]["extension"]=="mp4"){
			//array_push($item,$i);
			if($s['urls'][$i]["size"]<$s['urls'][$imin]["size"]){
				$imin=$i;
			}
		}
